...
import API_BASE_URL from '../config'; // Importing the base URL
...
function Home() {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/notes/${userId}`);
  ...
  const handleSaveNote = async () => {
    if (note.trim()) {
      const userId = user._id;
      try {
        const response = await axios.post(`${API_BASE_URL}/api/notes`, { userId, text: note });
  ...
  const handleDownloadFile = async (fileId, fileName) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/documents/download/${fileId}`, {
        responseType: 'blob',
      });
  ...
  const handleUpdateNote = async (id) => {
    if (editNote.trim()) {
      try {
        const response = await axios.put(`${API_BASE_URL}/api/notes/${id}`, { text: editNote });
